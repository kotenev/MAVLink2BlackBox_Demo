...
public class matrixpilot{}
 class CommunicationChannel extends AdvancedProtocol implements GroundControl.CommunicationInterface, MicroAirVehicle.CommunicationInterface {}
class GroundControl implements  InJAVA, InCS{
interface CommunicationInterface extends GroundControlHandledPacks, CommonPacks {}interface GroundControlHandledPacks  { 
/**
...
@A byte  target_system;//System ID
@A byte  target_component;//Component ID
@A short  func_index;//Function index
@A short  func_count;//Total count of functions
@A short  data_address;//Address in the flexifunction data, Set to 0xFFFF to use address in target memory
@A short  data_size;//Size of the 
@D (48)byte []  data;//Settings data
}
...
@A byte  target_component;//Component ID
@A short  func_index;//Function index
@A short  result;//result of acknowledge, 0=fail, 1=good
}
...
@A byte  target_system;//System ID
@A byte  target_component;//Component ID
@A byte  directory_type;//0=inputs, 1=outputs
@A byte  start_index;//index of first directory entry to write
@A byte  count;//count of directory entries to write
@D (48)byte []  directory_data;//Settings data
}
...
@A byte  target_system;//System ID
@A byte  target_component;//Component ID
@A byte  command_type;//Flexifunction command type
}

/**
Acknowldge sucess or failure of a flexifunction command*/
@id(158) class FLEXIFUNCTION_COMMAND_ACK{
@A short  command_type;//Command acknowledged
@A short  result;//result of acknowledge
}

/**
Backwards compatible MAVLink version of SERIAL_UDB_EXTRA - F2: Format Part A*/
@id(170) class SERIAL_UDB_EXTRA_F2_A{
@A int  sue_time;//Serial UDB Extra Time
@A byte  sue_status;//Serial UDB Extra Status
...
@A short  sue_waypoint_index;//Serial UDB Extra Waypoint Index
...
@A short  sue_cog;//Serial UDB Extra GPS Course Over Ground
short  sue_sog;//Serial UDB Extra Speed Over Ground
@A short  sue_cpu_load;//Serial UDB Extra CPU Load
@A short  sue_air_speed_3DIMU;//Serial UDB Extra 3D IMU Air Speed
...
@A int  sue_flags;//Serial UDB Extra Status Flags
...
@A byte  sue_ROLL_STABILIZATION_AILERONS;//Serial UDB Extra Roll Stabilization with Ailerons Enabled
@A byte  sue_ROLL_STABILIZATION_RUDDER;//Serial UDB Extra Roll Stabilization with Rudder Enabled
@A byte  sue_PITCH_STABILIZATION;//Serial UDB Extra Pitch Stabilization Enabled
@A byte  sue_YAW_STABILIZATION_RUDDER;//Serial UDB Extra Yaw Stabilization using Rudder Enabled
@A byte  sue_YAW_STABILIZATION_AILERON;//Serial UDB Extra Yaw Stabilization using Ailerons Enabled
@A byte  sue_AILERON_NAVIGATION;//Serial UDB Extra Navigation with Ailerons Enabled
@A byte  sue_RUDDER_NAVIGATION;//Serial UDB Extra Navigation with Rudder Enabled
@A byte  sue_ALTITUDEHOLD_STABILIZED;//Serial UDB Extra Type of Alitude Hold when in Stabilized Mode
@A byte  sue_ALTITUDEHOLD_WAYPOINT;//Serial UDB Extra Type of Alitude Hold when in Waypoint Mode
@A byte  sue_RACING_MODE;//Serial UDB Extra Firmware racing mode enabled
}
...
@A byte  sue_WIND_ESTIMATION;//Serial UDB Extra Wind Estimation Enabled
@A byte  sue_GPS_TYPE;//Serial UDB Extra Type of GPS Unit
@A byte  sue_DR;//Serial UDB Extra Dead Reckoning Enabled
@A byte  sue_BOARD_TYPE;//Serial UDB Extra Type of UDB Hardware
@A byte  sue_AIRFRAME;//Serial UDB Extra Type of Airframe
short  sue_RCON;//Serial UDB Extra Reboot Register of DSPIC
short  sue_TRAP_FLAGS;//Serial UDB Extra  Last dspic Trap Flags
@A int  sue_TRAP_SOURCE;//Serial UDB Extra Type Program Address of Last Trap
short  sue_osc_fail_count;//Serial UDB Extra Number of Ocillator Failures
@A byte  sue_CLOCK_CONFIG;//Serial UDB Extra UDB Internal Clock Configuration
@A byte  sue_FLIGHT_PLAN_TYPE;//Serial UDB Extra Type of Flight Plan
}

/**
Backwards compatible version of SERIAL_UDB_EXTRA F15 format*/
@id(179) class SERIAL_UDB_EXTRA_F15{
@D (40)@A byte []  sue_ID_VEHICLE_MODEL_NAME;//Serial UDB Extra Model Name Of Vehicle
@D (20)@A byte []  sue_ID_VEHICLE_REGISTRATION;//Serial UDB Extra Registraton Number of Vehicle
}

/**
Backwards compatible version of SERIAL_UDB_EXTRA F16 format*/
@id(180) class SERIAL_UDB_EXTRA_F16{
@D (40)@A byte []  sue_ID_LEAD_PILOT;//Serial UDB Extra Name of Expected Lead Pilot
@D (70)@A byte []  sue_ID_DIY_DRONES_URL;//Serial UDB Extra URL of Lead Pilot or Team
}
...
@A int  time_boot_ms;//Timestamp (milliseconds since system boot)
int  alt_gps;//GPS altitude in meters, expressed as * 1000 (millimeters), above MSL
...
@A byte  sue_aileron_output_channel;//SUE aileron output channel
@A byte  sue_aileron_reversed;//SUE aileron reversed
@A byte  sue_elevator_output_channel;//SUE elevator output channel
@A byte  sue_elevator_reversed;//SUE elevator reversed
@A byte  sue_throttle_output_channel;//SUE throttle output channel
@A byte  sue_throttle_reversed;//SUE throttle reversed
@A byte  sue_rudder_output_channel;//SUE rudder output channel
@A byte  sue_rudder_reversed;//SUE rudder reversed
}

/**
Backwards compatible version of SERIAL_UDB_EXTRA F20 format*/
@id(186) class SERIAL_UDB_EXTRA_F20{
@A byte  sue_number_of_inputs;//SUE Number of Input Channels
