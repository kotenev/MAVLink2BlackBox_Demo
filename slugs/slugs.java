...
@A byte  sensLoad;//Sensor DSC Load
@A byte  ctrlLoad;//Control DSC Load
@A short  batVolt;//Battery Voltage in millivolts
}
...
@A byte  fromWP;//Origin WP
@A byte  toWP;//Destination WP
@A short  h_c;//Commanded altitude in 0.1 m
}
...
@A byte  year;//Year reported by Gps 
@A byte  month;//Month reported by Gps 
@A byte  day;//Day reported by Gps 
@A byte  hour;//Hour reported by Gps 
@A byte  min;//Min reported by Gps 
@A byte  sec;//Sec reported by Gps  
@A byte  clockStat;//Clock Status. See table 47 page 211 OEMStar Manual  
@A byte  visSat;//Visible satellites reported by Gps  
@A byte  useSat;//Used satellites in Solution  
@A byte  GppGl;//GPS+GLONASS satellites in Solution  
@A byte  sigUsedMask;//GPS and GLONASS usage mask (bit 0 GPS_used? bit_4 GLONASS_used?)
@A byte  percentUsed;//Percent used GPS
}
...
@A byte  target;//The system setting the commands
...
@A byte  target;//The system setting the commands
@A short  bitfieldPt;//Bitfield containing the passthrough configuration, see CONTROL_SURFACE_FLAG ENUM.
}

/**
Orders generated to the SLUGS camera mount. */
@id(184) class SLUGS_CAMERA_ORDER{
@A byte  target;//The system reporting the action
...
@A byte  target;//The system setting the commands
@A byte  idSurface;//ID control surface send 0: throttle 1: aileron 2: elevator 3: rudder
...
@A byte  target;//The system setting the commands
@A byte  idOrder;//ID 0: brightness 1: aperture 2: iris 3: ICR 4: backlight
@A byte  order;//1: up/on 2: down/off 3: auto/reset/no action
}
...
@A byte  option1;//Option 1
@A byte  option2;//Option 2
@A byte  option3;//Option 3
}

/**
Transmits the readings from the voltage and current sensors*/
@id(191) class VOLT_SENSOR{
@A byte  r2Type;//It is the value of reading 2: 0 - Current, 1 - Foreward Sonar, 2 - Back Sonar, 3 - RPM
@A short  voltage;//Voltage in uS of PWM. 0 uS = 0V, 20 uS = 21.5V 
/**
...
@A byte  zoom;//The actual Zoom Value
...
@A byte  target;//The ID system reporting the action
...
@A short  csFails;//Number of times checksum has failed
/**
...
@A byte  gpsQuality;
@A byte  msgsType;//Indicates if GN, GL or GP messages are being received
@A byte  posStatus;//A = data valid, V = data invalid
...
@A byte  timeStatus;//The Time Status. See Table 8 page 27 Novatel OEMStar Manual
@A int  receiverStatus;//Status Bitfield. See table 69 page 350 Novatel OEMstar Manual
@A byte  solStatus;//solution Status. See table 44 page 197
@A byte  posType;//position type. See table 43 page 196
@A byte  velType;//velocity type. See table 43 page 196
float  posSolAge;//Age of the position solution in seconds
@A short  csFails;//Times the CRC has failed since boot
}
...
numbers on startup*/
@id(197) class BOOT{
@A int  version;//The onboard software version
}
}}
