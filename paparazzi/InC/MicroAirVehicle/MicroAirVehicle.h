...
extern void c_CommunicationChannel_on_SCRIPT_ITEM_180(Bounds_Inside * bi, Pack * pack);
/**
...
extern void c_CommunicationChannel_on_SCRIPT_REQUEST_181(Bounds_Inside * bi, Pack * pack);
/**
*Request the overall list of mission items from the system/component.*/
extern void c_CommunicationChannel_on_SCRIPT_REQUEST_LIST_182(Bounds_Inside * bi, Pack * pack);
/**
*This message is emitted as response to SCRIPT_REQUEST_LIST by the MAV to get the number of mission scripts*/
extern void c_CommunicationChannel_on_SCRIPT_COUNT_183(Bounds_Inside * bi, Pack * pack);
/**
*This message informs about the currently active SCRIPT.*/
extern void c_CommunicationChannel_on_SCRIPT_CURRENT_184(Bounds_Inside * bi, Pack * pack);
/**
...
inline static e_MAV_MODE _en__Z(UMAX id)
{
    switch(id)
    {
        case 0:
...
inline static e_MAV_CMD _en__j(UMAX id)
{
    switch(id)
    {
        case 0:
...
inline static UMAX _id__j(e_MAV_CMD en)
{
    switch(en)
    {
...
{
    uint8_t * data = src->data;
    return  _en__j(get_bits(data, 248, 7));
}
...
{
    uint8_t * data = src->data;
    return  _en__j(get_bits(data, 0, 7));
}
...
{
    uint8_t * data = src->data;
    return  _en__Z(get_bits(data, 328, 4));
}
...
{
    uint8_t * data = src->data;
    return  _en__Z(get_bits(data, 640, 4));
}
...
INLINER uint16_t p180_seq_GET(Pack * src)//Sequence
{
    uint8_t * data = src->data;
    return ((get_bytes(data,  0, 2)));
}
...
INLINER uint8_t p180_target_component_GET(Pack * src)//Component ID
{
    uint8_t * data = src->data;
    return ((get_bytes(data,  3, 1)));
}
INLINER char16_t * p180_name_GET(Bounds_Inside * src, char16_t *  dst, int32_t pos) //The name of the mission script, NULL terminated.
{
    uint8_t * data = src->base.pack->data;
...
INLINER int32_t p180_name_LEN(Bounds_Inside * src)
{
    return (src->base.field_bit !=  32 && !try_visit_field(src, 32)  ||  !try_visit_item(src, 0)) ? 0 : src->items;
}
INLINER  char16_t* p180_name_TRY_(Bounds_Inside * src)
{
    if(src->base.field_bit !=  32 && !try_visit_field(src, 32)  ||  !try_visit_item(src, 0)) return NULL;
    char16_t * ret = p180_name_GET(src, malloc((src->items + 1) * 2), 0);
    ret[src->items] = 0;//the terminating null character.
    return ret;
}
INLINER uint16_t p181_seq_GET(Pack * src)//Sequence
{
    uint8_t * data = src->data;
    return ((get_bytes(data,  0, 2)));
}
INLINER uint8_t p181_target_system_GET(Pack * src)//System ID
{
    uint8_t * data = src->data;
    return ((get_bytes(data,  2, 1)));
}
INLINER uint8_t p181_target_component_GET(Pack * src)//Component ID
{
    uint8_t * data = src->data;
    return ((get_bytes(data,  3, 1)));
}
INLINER uint8_t p182_target_system_GET(Pack * src)//System ID
{
    uint8_t * data = src->data;
    return ((get_bytes(data,  0, 1)));
}
INLINER uint8_t p182_target_component_GET(Pack * src)//Component ID
{
    uint8_t * data = src->data;
    return ((get_bytes(data,  1, 1)));
}
INLINER uint16_t p183_count_GET(Pack * src)//Number of script items in the sequence
{
    uint8_t * data = src->data;
    return ((get_bytes(data,  0, 2)));
}
...
INLINER uint16_t p184_seq_GET(Pack * src)//Active Sequence
{
    uint8_t * data = src->data;
    return ((get_bytes(data,  0, 2)));
}
