...
public class BitUtils {
	private final static char[] Ol = {0, 1, 3, 7, 15, 31, 63, 127, 255};
	private final static char[] lO = {0, 128, 192, 224, 240, 248, 252, 254, 255};


	public static final int hibit(int i) {
		int ret = 0;
		if (0xFFFF < i)
		{
			i >>= 16;
	...
	public static long get_bits(byte[] src, int bit, int bits) {
		...
		if (64 < bits) bits = 64;
			...
			if (bit + bits < 9) return (src[src_byte] & 0xFF) >> bit & Ol[bits];
		long dst = 0;
	...
	public static void set_0(byte[] dst, int bit, int bits) {
		...
		if (8 < bit + bits)
		{
			...
			if ((bits >>= 3) == 0) return;
			for (int i = bits & 7; 0 < i--; dst[dst_byte] = 0, dst_byte++) ;
			for (int i = dst_byte + (bits >> 3); dst_byte <= --i; ) dst[i] = 0;
		}
