...
INLINER uint8_t  hi_bit(int32_t i)
{
    int32_t ret = 0;
    if(0xFFFF < i)
    {
...
static inline uint64_t doubleToLongBits(double val) { return *(uint64_t*)&val; }
...
INLINER uint16_t bits2bytes(int32_t bits) { return bits < 1 ? 0 : 1 + ((bits - 1) >> 3); }




INLINER UMAX get_bytes(uint8_t* src, int32_t byte, int32_t bytes)
{
#ifdef   UINT64_MAX
    int32_t hi = 0;
#endif
...
INLINER int32_t set_bytes(const UMAX src, const int32_t bytes, uint8_t* dst, const int32_t byte)
{
#ifdef   UINT64_MAX
...
INLINER UMAX get_bits(uint8_t* src, uint32_t bit, uint16_t bits)
{
    src += bit >> 3;
    bit &= 7;
...
INLINER void set_0(uint8_t* dst, uint32_t bit, uint32_t bits)
{
    dst += bit >> 3;
    bit &= 7;
    if(8 < bit + bits)
    {
        ...
        for(UMAX* umax = (UMAX*)dst + (bits / sizeof(UMAX) - 1); (UMAX*)dst <= umax; umax--)
            *umax = 0;
    }
    else *dst &= Ol[bit] | lO[8 - (bit + bits)];
}

INLINER void set_bits(UMAX src, uint8_t bits, uint8_t* dst, uint32_t bit)
{
    dst += bit >> 3;
    bit &= 7;
    if(8 < bit + bits)
