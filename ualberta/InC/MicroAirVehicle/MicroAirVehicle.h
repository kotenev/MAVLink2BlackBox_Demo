...
extern void c_CommunicationChannel_on_NAV_FILTER_BIAS_220(Bounds_Inside * bi, Pack * pack);
/**
*Complete set of calibration parameters for the radio*/
extern void c_CommunicationChannel_on_RADIO_CALIBRATION_221(Bounds_Inside * bi, Pack * pack);
/**
*System status specific to ualberta uav*/
extern void c_CommunicationChannel_on_UALBERTA_SYS_STATUS_222(Bounds_Inside * bi, Pack * pack);
/**
...
inline static e_MAV_MODE _en__I(UMAX id)
{
    switch(id)
    {
        case 0:
...
inline static UMAX _id__I(e_MAV_MODE en)
{
    switch(en)
    {
...
inline static e_MAV_MISSION_TYPE _en__L(UMAX id)
{
    switch(id)
    {
        case 0:
...
inline static UMAX _id__L(e_MAV_MISSION_TYPE en)
{
    switch(en)
    {
...
inline static e_MAV_CMD _en__S(UMAX id)
{
    switch(id)
    {
        case 0:
...
inline static UMAX _id__S(e_MAV_CMD en)
{
    switch(en)
    {
...
{
    uint8_t * data = src->data;
    return  _en__S(get_bits(data, 248, 7));
}
...
{
    uint8_t * data = src->data;
    return  _en__S(get_bits(data, 0, 7));
}
...
{
    uint8_t * data = src->data;
    return  _en__I(get_bits(data, 328, 4));
}
...
{
    uint8_t * data = src->data;
    return  _en__I(get_bits(data, 640, 4));
}
...
INLINER uint64_t p220_usec_GET(Pack * src)//Timestamp (microseconds)
{
    uint8_t * data = src->data;
    return ((get_bytes(data,  0, 8)));
}
INLINER float p220_accel_0_GET(Pack * src)//b_f[0]
{
    uint8_t * data = src->data;
    return (intBitsToFloat(get_bytes(data,  8, 4)));
}
INLINER float p220_accel_1_GET(Pack * src)//b_f[1]
{
    uint8_t * data = src->data;
    return (intBitsToFloat(get_bytes(data,  12, 4)));
}
INLINER float p220_accel_2_GET(Pack * src)//b_f[2]
{
    uint8_t * data = src->data;
    return (intBitsToFloat(get_bytes(data,  16, 4)));
}
INLINER float p220_gyro_0_GET(Pack * src)//b_f[0]
{
    uint8_t * data = src->data;
    return (intBitsToFloat(get_bytes(data,  20, 4)));
}
INLINER float p220_gyro_1_GET(Pack * src)//b_f[1]
{
    uint8_t * data = src->data;
    return (intBitsToFloat(get_bytes(data,  24, 4)));
}
INLINER float p220_gyro_2_GET(Pack * src)//b_f[2]
{
    uint8_t * data = src->data;
    return (intBitsToFloat(get_bytes(data,  28, 4)));
}
INLINER uint16_t* p221_aileron_GET(Pack * src, uint16_t*  dst, int32_t pos) //Aileron setpoints: left, center, right
{
    uint8_t * data = src->data;
    for(int32_t BYTE = 0, dst_max = pos + 3; pos < dst_max ; pos++,  BYTE += 2)
        dst[pos] = ((get_bytes(data,  BYTE, 2)));
    return dst;
}

static const  uint32_t p221_aileron_LEN = 3; //return array length

INLINER  uint16_t*  p221_aileron_GET_(Pack * src) {return p221_aileron_GET(src, malloc(3 * sizeof(uint16_t)), 0);}
INLINER uint16_t* p221_elevator_GET(Pack * src, uint16_t*  dst, int32_t pos) //Elevator setpoints: nose down, center, nose up
{
    uint8_t * data = src->data;
    for(int32_t BYTE = 6, dst_max = pos + 3; pos < dst_max ; pos++,  BYTE += 2)
        dst[pos] = ((get_bytes(data,  BYTE, 2)));
    return dst;
}

static const  uint32_t p221_elevator_LEN = 3; //return array length

INLINER  uint16_t*  p221_elevator_GET_(Pack * src) {return p221_elevator_GET(src, malloc(3 * sizeof(uint16_t)), 0);}
INLINER uint16_t* p221_rudder_GET(Pack * src, uint16_t*  dst, int32_t pos) //Rudder setpoints: nose left, center, nose right
{
    uint8_t * data = src->data;
    for(int32_t BYTE = 12, dst_max = pos + 3; pos < dst_max ; pos++,  BYTE += 2)
        dst[pos] = ((get_bytes(data,  BYTE, 2)));
    return dst;
}

static const  uint32_t p221_rudder_LEN = 3; //return array length

INLINER  uint16_t*  p221_rudder_GET_(Pack * src) {return p221_rudder_GET(src, malloc(3 * sizeof(uint16_t)), 0);}
INLINER uint16_t* p221_gyro_GET(Pack * src, uint16_t*  dst, int32_t pos) //Tail gyro mode/gain setpoints: heading hold, rate mode
{
    uint8_t * data = src->data;
    for(int32_t BYTE = 18, dst_max = pos + 2; pos < dst_max ; pos++,  BYTE += 2)
        dst[pos] = ((get_bytes(data,  BYTE, 2)));
    return dst;
}

static const  uint32_t p221_gyro_LEN = 2; //return array length

INLINER  uint16_t*  p221_gyro_GET_(Pack * src) {return p221_gyro_GET(src, malloc(2 * sizeof(uint16_t)), 0);}
INLINER uint16_t* p221_pitch_GET(Pack * src, uint16_t*  dst, int32_t pos) //Pitch curve setpoints (every 25%)
{
    uint8_t * data = src->data;
    for(int32_t BYTE = 22, dst_max = pos + 5; pos < dst_max ; pos++,  BYTE += 2)
        dst[pos] = ((get_bytes(data,  BYTE, 2)));
    return dst;
}

static const  uint32_t p221_pitch_LEN = 5; //return array length

INLINER  uint16_t*  p221_pitch_GET_(Pack * src) {return p221_pitch_GET(src, malloc(5 * sizeof(uint16_t)), 0);}
INLINER uint16_t* p221_throttle_GET(Pack * src, uint16_t*  dst, int32_t pos) //Throttle curve setpoints (every 25%)
{
    uint8_t * data = src->data;
    for(int32_t BYTE = 32, dst_max = pos + 5; pos < dst_max ; pos++,  BYTE += 2)
        dst[pos] = ((get_bytes(data,  BYTE, 2)));
    return dst;
}

static const  uint32_t p221_throttle_LEN = 5; //return array length

INLINER  uint16_t*  p221_throttle_GET_(Pack * src) {return p221_throttle_GET(src, malloc(5 * sizeof(uint16_t)), 0);}
INLINER uint8_t p222_mode_GET(Pack * src)//System mode, see UALBERTA_AUTOPILOT_MODE ENUM
{
    uint8_t * data = src->data;
    return ((get_bytes(data,  0, 1)));
}
INLINER uint8_t p222_nav_mode_GET(Pack * src)//Navigation mode, see UALBERTA_NAV_MODE ENUM
{
    uint8_t * data = src->data;
    return ((get_bytes(data,  1, 1)));
}
INLINER uint8_t p222_pilot_GET(Pack * src)//Pilot mode, see UALBERTA_PILOT_MODE
{
    uint8_t * data = src->data;
    return ((get_bytes(data,  2, 1)));
}
