...
namespace org.unirail.BlackBox
{
    public static class BitUtils
    {
        ...
        public static uint hibit(uint i)
        {
            uint ret = 0;
            if(0xFFFF < i)
            {
        ...
        public static int set_bytes(ulong src, int bytes, byte[] dst, int BYTE)
        {
        ...
        public static unsafe uint FloatToInt32Bits(float value) { return *(uint*) &value; }
        public static unsafe float Int32BitsToFloat(uint value) { return *(float*) &value; }
        ...
        public static ulong get_bits(byte[] src, int bit, int bits)
        {
            ...
            if(64 < bits) bits = 64;
                ...
                return (ulong)((src[src_byte] >> bit) & Ol[bits]);
            ulong dst = 0;
        ...
        public static void set_0(byte[] bytes, int bit, int bits)
        {
            var dst_byte = bit >> 3;
            bit &= 7;
            if(8 < bit + bits)
            {
                if(0 < bit)
                {
                    bytes[dst_byte] &= Ol[bit];
                    dst_byte++;
                    if((bits -= 8 - bit) == 0) return;
                }
                if(0 < (bits & 7)) bytes[dst_byte + (bits >> 3)] &= lO[8 - (bits & 7)];
                if((bits >>= 3) == 0) return;
                for(var i = bits & 7; 0 < i--; bytes[dst_byte] = 0, dst_byte++) ;
                for(var i = dst_byte + (bits >> 3); dst_byte <= --i;) bytes[i] = 0;
            }
            else bytes[dst_byte] &= (byte)(Ol[bit] | lO[8 - (bit + bits)]);
        }
        ...
        public static void set_bits(ulong src, int bits, byte[] dst, int bit)
        {
            ...
            if(8 < bit + bits)
            {
                ...
                for(int BYTE = 0, bytes = bits >> 3; BYTE < bytes; BYTE++, usrc >>= 8) dst[dst_byte++] = (byte)(usrc & 0xFF);
                if(0 < (bits &= 7))
